...
class Solution {
    public int findInMountainArray(int target, MountainArray mountainArr) {
        ...
        final int peakIndex = peakIndexMountainArray(mountainArr, 0, n - 1);

        final int leftIndex = searchLeft(mountainArr, target, 0, peakIndex);
        if(mountainArr.get(leftIndex) == target) return leftIndex;

        final int rightIndex = searchRight(mountainArr, target, peakIndex + 1, n - 1);
        if(mountainArr.get(rightIndex) == target) return rightIndex;

        return -1;
    }

    private int peakIndexMountainArray(MountainArray A, int l, int r){
        while(l < r){
            final int m = (l + r) / 2;
